...
// COL adjustment toggled via header checkbox.
...
function draw() {
  ...
  const rScale = d3.scaleSqrt()
    .domain([0, d3.max(nodes, d => d.count)]).range([12, 135]);
  ...
  svg.append("g").selectAll("circle").data(nodes).join("circle")
    ...
    .attr("fill", d => COLOR(d.family)).attr("opacity", 0.75)
  ...
  if (rightSide) {
    const poolNote = colAdjusted
      ...
      : "Salaries not COL-adjusted.<br>Thresholds are averages<br>across all campuses.";
    rightSide.innerHTML = `
