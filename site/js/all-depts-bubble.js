// System-wide bubble chart: one bubble per dept_name, sized by headcount, y ≈ median salary.
...
function draw() {
  ...
  const nodes = [...depts.entries()].map(([dept, s]) => ({ dept, ...s }));
  ...
  const COLOR = d3.scaleOrdinal(d3.schemeTableau10).domain(nodes.map(d => d.dept));
  ...
  const axisG = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);

  axisG.call(d3.axisLeft(yScale).ticks(8).tickFormat(d => "$" + d3.format(",.0f")(d)))
  ...
  svg.append("g").selectAll("circle").data(nodes).join("circle")
    ...
    .attr("fill", d => COLOR(d.dept)).attr("opacity", 0.75)
    .on("mousemove", function(event, d) {
      const fmt = d3.format(",.0f");
      tooltip.style("opacity", 1).html(
        `<strong>${d.dept}</strong><br>n = ${d3.format(",")(d.count)}<br>` +
  ...
  svg.append("g").selectAll("text").data(nodes.filter(d => d.r > 20)).join("text")
  ...
  if (rightSide) {
    ...
    rightSide.innerHTML = `
      <div style="font-size:0.8rem;color:#aaa;margin-bottom:0.6rem;font-weight:bold;">CU System</div>
