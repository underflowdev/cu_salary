// ECDF: one line per campus, X = COL-adjusted annual salary, Y = fraction of employees below that value.
// Full-time employees only. Wage threshold markers are vertical lines at average COL-adjusted values.
...
Promise.all([
  ...
  // Build per-campus sorted salary arrays (COL-adjusted)
  const campusData = chunks.map(({ key, rows }) => {
    const col = meta[key].cost_of_living;
    const salaries = rows
      .filter(r => r.full_time_pct === "100" && parseSalary(r.total) > 0)
      .map(r => parseSalary(r.total) / col)
      .sort(d3.ascending);
    return { key, salaries };
  });

  draw(campusData, meta);
});

function draw(campusData, meta) {
  ...
  const campusMetas = CAMPUSES.map(c => meta[c.key]);
  ...
  campusData.forEach(({ key, salaries }) => {
    ...
    // Prepend a zero point so the line starts at the left axis
  ...
  g.append("rect")
    ...
    .on("mousemove", function(event) {
      ...
      if (tx + tw > vw - pad) tx = event.clientX - tw - pad;
      if (ty + th > vh - pad) ty = event.clientY - th - pad;
  ...
  if (rightSide) {
    rightSide.innerHTML = `
      ...
      <div style="font-size:0.72rem;color:#555;margin-top:1rem;line-height:1.6;">
        Salary COL-adjusted<br>per campus.<br>
        Thresholds = avg<br>across campuses.<br>
        Hourly × 2,080 hrs.<br><br>
